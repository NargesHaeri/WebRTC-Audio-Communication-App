...
متد `sendTrack` وظیفه ارسال داده‌ها رو از طریق ترک مشخص شده (با `peerId`) بر عهده داره. اول از همه چک می‌کنه که آیا ترک برای همتا وجود داره یا نه. اگر ترک باز نباشه، می‌ره و یه پیام هشدار می‌زاره. اگه همه چیز درست بود، هدر RTP رو با اطلاعاتی مثل شماره توالی و زمان درست می‌کنه.

بعدش، این هدر رو به داده‌ها اضافه می‌کنه و بسته رو برای ارسال آماده می‌کنه. این بسته رو با استفاده از متد `send` به ترک می‌فرسته. اگه مشکلی پیش بیاد، خطا رو توی لاگ ثبت می‌کنه. به این ترتیب، ارسال داده‌ها رو به خوبی مدیریت می‌کنه.
